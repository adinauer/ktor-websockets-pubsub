...
class ChatApplication {
    ...
    private suspend fun receivedMessage(id: String, command: String) {
        ...
        when {
            ...
            command.startsWith("/join") -> server.joinRoom(id, command.removePrefix("/join").trim())
            command.startsWith("/say") -> server.sayToRoom(id, command.removePrefix("/say").trim().split(" ").first(), command.removePrefix("/say").trim().split(" ").drop(1).joinToString(" "))
