...
class ChatServer {
    ...
    val consumers = ConcurrentHashMap<String, MutableList<Consumer<ByteArray>>>()
    ...
    suspend fun memberJoin(member: String, socket: WebSocketSession) {
        ...
        socket.send(Frame.Text("welcome"))

        val consumerList = consumers.computeIfAbsent(member) { CopyOnWriteArrayList<Consumer<ByteArray>>() }
        consumerList.add(
            pulsar.newConsumer().topic("general").subscriptionName(UUID.randomUUID().toString()).messageListener { consumer, message ->
                GlobalScope.launch(Dispatchers.IO) {
                    members[member]?.send(Frame.Text(String(message.data)))
                }
            }.subscribe()
        )
    }

    suspend fun joinRoom(member: String, room: String) {
        val consumerList = consumers.computeIfAbsent(member) { CopyOnWriteArrayList<Consumer<ByteArray>>() }
        consumerList.add(
                pulsar.newConsumer().topic(room).subscriptionName(UUID.randomUUID().toString()).messageListener { consumer, message ->
                    GlobalScope.launch(Dispatchers.IO) {
                        members[member]?.send(Frame.Text(String(message.data)))
                    }
                }.subscribe()
    ...
    suspend fun sayToRoom(member: String, room: String, message: String) {
        println("sending message to toom ${room}: ${message}")
        pulsar.newProducer().topic(room).create().send(message.toByteArray())
    }
    ...
    suspend fun message(sender: String, message: String) {
        ...
        PulsarClient.builder()
                ...
                .newProducer()
                .topic("my-topic")
                .producerName("ktor_pulsar_producer")
                .create()
                .newMessage()
